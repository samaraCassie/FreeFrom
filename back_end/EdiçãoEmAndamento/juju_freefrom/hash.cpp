...
void Hash::busca(Aluno& aluno, bool& busca)
{
    ...
    while (estrutura[local].obterId() != -1)
    {
        ...
        cout << setw(5) << "Preco: " << aluno.obterPreco() << endl;
        cout << "\n*----------******************----------*\n\n";
        break;
        }
        local = (local+1) % max_posicoes;
...
void Hash::imprimir()
{
    ...
    for (int i = 0; i < max_posicoes; i++)
    {
        if (estrutura[i].obterId() > 0)
        {
            ...
            cout << setw (4) << "R$" <<  estrutura[i].obterPreco() << endl;
        }
    }
    cout << "\n*----------******************----------*\n\n";
}
void Hash::editar()
{
    ...
    cout << "Deseja editar o Nome ou o preco?\n"
    cout << "Nome[N]"
    cout << "Preco[P]"
    cin >> editar;
        if (editar == N || editar == n){
            estrutura[local].obterNome()
             estrutura[local] = nome;
             cout << "Digite o novo nome: "
             cin >> nome;
             Aluno aluno(id, nome, preco);
             alunoHash.editar(aluno);
        }
    }
}
